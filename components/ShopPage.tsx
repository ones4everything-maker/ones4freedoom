...
const MOCK_ITEMS: ProductItem[] = [
    ...
    },
    {
        ...
        theme: 'purple',
        tags: ['pants', 'denim', 'black', 'bottoms'],
...
async function fetchProducts(): Promise<ProductItem[]> {
    // If domain isn't set, return mock items after delay
    if (!SHOPIFY_SETUP.domain) {
    ...
    try {
        ...
        return json.data.products.edges.map((edge: any) => {
            ...
            if (p.tags.some((t: string) => t.toLowerCase().includes('purple'))) theme = 'purple';
            else if (p.tags.some((t: string) => t.toLowerCase().includes('orange'))) theme = 'orange';
            else if (p.tags.some((t: string) => t.toLowerCase().includes('cyan'))) theme = 'cyan';
            else {
            ...
            <div className="bg-[#151621] w-full max-w-2xl rounded-2xl border border-cyan-500/30 shadow-[0_0_50px_rgba(6,182,212,0.2)] overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-8 overflow-y-auto space-y-8 text-slate-300 custom-scrollbar">
                    <section>
                        ...
                        <p className="mb-3">To load real products, edit the <code className="text-cyan-400 bg-black/30 px-1.5 py-0.5 rounded border border-white/5">SHOPIFY_SETUP</code> object.</p>
                    </section>
                </div>
...
const Navbar = ({ 
    ...
    onNavigate: (view: 'immersive') => void
}) => {
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0B0C15]/80 backdrop-blur-md border-b border-white/10 h-16 md:h-20 flex items-center px-4 md:px-8 justify-between shadow-2xl">
            ...
            <div className="flex items-center gap-4 md:gap-6">
                ...
                <button onClick={() => onNavigate('immersive')}>
                    <h1 className="text-xl md:text-2xl font-bold text-white tracking-widest font-sans hover:text-cyan-400 transition-colors">{APP_CONFIG.siteName}</h1>
                </button>
            </div>
                ...
                <div className="p-6 h-full flex flex-col">
                    ...
                    <div className="space-y-2">
                         <button 
                            ...
                            className="w-full text-left py-3 px-4 rounded-xl text-cyan-400 bg-white/5 font-medium flex items-center justify-between group mb-4"
                        >
                            Return Home
                    ...
                    <div className="p-6 md:p-8 flex-grow overflow-y-auto custom-scrollbar">
                        ...
                        <div className="pt-2">
                            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                                <span>Reviews</span>
                                <span className="text-sm font-normal text-slate-500 bg-white/5 px-2 py-0.5 rounded-full">
                                    {item.reviews.length}
                                </span>
                            </h3>
                            
                            {/* Review Form */}
                            <form onSubmit={handleSubmitReview} className="mb-8 bg-white/5 p-4 sm:p-5 rounded-2xl border border-white/5 shadow-inner">
                                <h4 className="text-sm font-bold text-slate-300 mb-4 uppercase tracking-wider">Write a review</h4>
                                
                                <div className="flex flex-col sm:flex-row gap-4 mb-4">
                                     <div className="flex flex-col gap-2">
                                        <span className="text-xs text-slate-500 font-bold">Rating</span>
                                        <div className="flex gap-1 bg-black/20 p-2 rounded-lg w-fit">
                                            {[1,2,3,4,5].map(star => (
                                                <button 
                                                    key={star} 
                                                    type="button" 
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-black/30 border border-white/10 rounded-xl p-4 text-white text-sm focus:outline-none focus:border-cyan-500/50 mb-4 h-24 resize-none transition-colors"
                                    required
                                />
                                <div className="flex justify-end">
                                    <button 
                                        type="submit" 
                                        className="w-full sm:w-auto px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-bold rounded-xl transition-all shadow-[0_4px_10px_rgba(6,182,212,0.2)] hover:shadow-[0_4px_20px_rgba(6,182,212,0.4)]"
                                    >
                                        Submit Review
                                    </button>
                                </div>
                            ...
                            <div className="space-y-4">
                                {item.reviews.length === 0 ? (
                                    ...
                                    [...item.reviews].reverse().map(review => (
                                        <div key={review.id} className="bg-white/5 p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-3">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-900/50 to-purple-900/50 flex items-center justify-center text-white font-bold text-sm border border-white/10 shadow-lg">
                                                ...
                                                <span className="text-xs text-slate-500 bg-black/20 px-2 py-1 rounded-full self-start">{review.date}</span>
                                            </div>
...
export const ShopPage = ({ onNavigate }: { onNavigate: (view: 'immersive') => void }) => {
    ...
    const filteredAndSortedItems = products
        .filter(item => {
            // Tag Filter
            // Matches any part of any tag or title with the active category
    ...
    return (
        <div className="min-h-screen pb-24 font-sans text-slate-200 bg-[#0B0C15]">
            {/* Navbar */}
            <Navbar 
                cartCount={cartCount} 
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                            <div>
                                <h2 className="text-3xl font-bold text-slate-200 uppercase tracking-wide mb-1">
                                    {activeTag === 'All' ? 'Collection' : activeTag}
                                </h2>
