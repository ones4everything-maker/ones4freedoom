  ...
  return (
    <div className="group relative bg-[#001B49]/80 md:bg-[#001B49]/60 border border-white/5 hover:border-cyan-400/50 rounded-xl overflow-hidden backdrop-blur-md transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(6,182,212,0.15)] flex flex-col h-full">
      ...
      <div className="aspect-square relative overflow-hidden bg-[#0F1729]/50">
        ...
        {/* Floating Tag */}
        {product.on_sale && (
          <div className="absolute top-2 right-2 z-20 bg-cyan-400 text-black text-[10px] sm:text-xs font-bold px-2 py-1 rounded-sm uppercase tracking-wider shadow-lg">
            Sale
          </div>
        )}
      </div>
      ...
      <div className="p-3 sm:p-4 relative z-20 flex-1 flex flex-col justify-between">
        <div>
          <h3 className="text-white font-medium text-sm sm:text-lg tracking-wide truncate group-hover:text-cyan-400 transition-colors duration-300" title={product.name}>
            {product.name}
          </h3>
          
          {/* Mobile-visible description */}
          <p className="mt-1 sm:mt-2 text-[10px] sm:text-xs text-slate-400 line-clamp-2 block md:hidden">
        ...
        <div className="flex justify-between items-end mt-3 sm:mt-4">
          <div className="flex flex-col">
            <span className="text-slate-500 text-[10px] sm:text-xs uppercase tracking-widest">Price</span>
            <div className="flex items-baseline gap-1 sm:gap-2">
              <span className="text-base sm:text-xl font-bold text-white font-display">${product.price}</span>
              {product.regular_price !== product.price && (
                <span className="text-[10px] sm:text-xs text-slate-500 line-through">${product.regular_price}</span>
              )}
            </div>
          </div>
        ...
        <div className="hidden md:block h-0 group-hover:h-auto overflow-hidden transition-all duration-500">
            <p className="pt-3 text-xs text-slate-400 line-clamp-2">
                {product.short_description.replace(/<[^>]*>?/gm, '')}
            </p>
            <div className="flex items-center gap-1 text-cyan-400 text-xs mt-2 uppercase tracking-wider font-bold cursor-pointer hover:underline">
